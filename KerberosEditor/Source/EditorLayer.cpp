...
namespace Kerberos
{
	...
	void EditorLayer::PrepareUniformBuffers()
	{
		...
		for (auto& [scene, params, perObject, skybox] : m_UniformBuffers)
		{
			...
			perObject = std::make_shared<UniformBuffer>(m_DynamicAlignment * MaxObjects);

			skybox = std::make_shared<UniformBuffer>(sizeof(SkyboxData));
		}
	}

	void EditorLayer::SetupDescriptors() 
	{
	...
	void EditorLayer::CreateVulkanResources()
	{
		...
		{
			m_ColorOutputDescriptorSet = VulkanContext::GenerateImGuiDescriptorSet(m_ColorSampler, m_ColorImageView);
			...
			m_ShadowMapDescriptorSet = VulkanContext::GenerateImGuiDescriptorSet(m_ColorSampler, m_ShadowMapImageView);
	...
	void EditorLayer::ResizeResources() 
	{
