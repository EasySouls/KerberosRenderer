...
std::vector<uint32_t> SlangCompiler::CompileToSpirv(const std::filesystem::path& filepath, const std::vector<ShaderEntryPoint>& entryPoints)
{
	...
	{
		Log::CoreError("Failed to find Slang profile 'spirv_1_6'");
	...
	{
		Log::CoreError("Failed to create Slang session");
	...
	if (!module)
	{
		...
		Log::CoreError("Shader compilation error: {}", msg);
		throw CompilationFailedException(std::string("Shader compilation error: ") + msg);
	}
	if (moduleDiagnostics)
	{
		Log::CoreWarn("Shader compilation diagnostics: {}", static_cast<const char*>(moduleDiagnostics->getBufferPointer()));
	}
	...
	if (entryPoints.empty())
	{
		...
		for (int32_t i = 0; i < definedEntryPointCount; ++i)
		{
			...
			{
				Log::CoreError("Failed to find defined entry point '{}' in shader module '{}'", i, moduleName);
				continue;
			}
	...
	else
	{
		...
		for (const ShaderEntryPoint& entryPoint : entryPoints)
		{
			...
			{
				Log::CoreError("Failed to find entry point '{}' in shader module '{}'", entryPointName, moduleName);
				continue;
			}
	...
	{
		Log::CoreError("Failed to create composite component type: {}", static_cast<const char*>(compositionDiagnostics->getBufferPointer()));
	...
	{
		Log::CoreWarn("Shader composition warning: {}", static_cast<const char*>(compositionDiagnostics->getBufferPointer()));
	}
	...
	if (SLANG_FAILED(linkResult))
	{
		...
		Log::CoreError("Shader linking error: {}", msg);
		throw CompilationFailedException(std::string("Shader linking error: ") + msg);
	}
	if (linkDiagnostics)
	{
		Log::CoreWarn("Shader linking diagnostics: {}", static_cast<const char*>(linkDiagnostics->getBufferPointer()));
	}
	...
	/*for (SlangUInt entryPointIndex = 0; entryPointIndex < layout->getEntryPointCount(); ++entryPointIndex)
	{
		...
		{
			Log::CoreError("Failed to get SPIR-V code for entry point '{}': {}", layout->getEntryPointByIndex(entryPointIndex)->getName(), static_cast<const char*>(diagnostics->getBufferPointer()));
	...
	Slang::ComPtr<IBlob> spirvBlob;
	{
		...
		{
			Log::CoreError("Failed to get SPIR-V code for shader '{}': {}", moduleName, static_cast<const char*>(diagnosticsBlob->getBufferPointer()));
		...
		if (diagnosticsBlob)
		{
			Log::CoreWarn("Shader compilation warning for '{}': {}", moduleName, static_cast<const char*>(diagnosticsBlob->getBufferPointer()));
		}
	}
